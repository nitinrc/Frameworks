...
public class MultiThreading {
	public void dataFetch() throws InterruptedException, ExecutionException {
		...
		callables.add(() -> {
			DataFetch objData = Config.context.getBean(DataFetch.class);
	...
	public HashMap<String, HashMap<String, String>> getElementIdentificationData(String[] arrFields, final String component) throws InterruptedException, ExecutionException {
		...
		for (final String item : arrFields) {
		    ...
		    callables.add(() -> {
				...
				mapElementParameters.put("Locator", objDataFetch.getMapPOM().get(component).get(item).get("Locator"));
				mapElementParameters.put("LocatorType", objDataFetch.getMapPOM().get(component).get(item).get("LocatorType"));
				mapElementParameters.put("ExpectedCondition", objDataFetch.getMapPOM().get(component).get(item).get("ExpectedCondition"));
				mapElementParameters.put("Timeout", objDataFetch.getMapPOM().get(component).get(item).get("Timeout"));
	...
	public void verifyElements(HashMap<String, HashMap<String, String>> mapElements) throws InterruptedException, ExecutionException {
		...
		for (final Entry<String, HashMap<String, String>> item : mapElements.entrySet()) {
		    ...
		    callables.add(() -> {
				Calendar cal1 = Calendar.getInstance();
				SimpleDateFormat timeOnly1 = new SimpleDateFormat("HH:mm:ss");
				FindElement objFindElement = Config.context.getBean(FindElement.class);
